...
export function registerAnalysisTools(server, client) {
    // ── Start Analysis ──
    server.registerTool("influiq_start_analysis", {
...
Returns: { analysisId, status, platform, creditsRemaining }`,
        ...
        try {
            const body = { platform: params.platform };
            if (params.url)
                body.url = params.url;
            if (params.username)
                body.channelId = params.username;
            if (params.brand_profile_id)
                body.brandProfileId = params.brand_profile_id;
            const result = await client.post("/analysis/start", body);
            return {
                content: [
                    {
                        type: "text",
    ...
    // ── Get Analysis ──
    server.registerTool("influiq_get_analysis", {
...
Returns: Full analysis object with profile, metrics, scores, and AI insights.`,
        ...
        try {
            const result = await client.get(`/analysis/${params.analysis_id}`);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.registerTool("influiq_get_analysis_progress", {
...
Returns: { status, progress (0-100), currentStep }`,
        ...
        try {
            const result = await client.get(`/analysis/${params.analysis_id}/progress`);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.registerTool("influiq_list_analyses", {
...
Returns: Paginated list of analyses with basic info (id, platform, username, status, date).`,
        ...
        try {
            ...
            const result = await client.get("/analysis/history", query);
            return {
                content: [
                    {
                        type: "text",
    ...
    // ── Dashboard Stats ──
    server.registerTool("influiq_get_dashboard_stats", {
...
Returns: { totalAnalyses, completedAnalyses, recentAnalyses, platformStats, analysisCredits }`,
        ...
        try {
            const result = await client.get("/analysis/dashboard/stats");
            return {
                content: [
                    {
                        type: "text",
